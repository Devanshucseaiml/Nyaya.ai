...
DROP POLICY IF EXISTS "Stories are viewable by everyone" ON legal_stories;
DROP POLICY IF EXISTS "Users can create stories" ON legal_stories;
DROP POLICY IF EXISTS "Users can update own stories" ON legal_stories;
DROP POLICY IF EXISTS "Users can delete own stories" ON legal_stories;
CREATE POLICY "Stories are viewable by everyone" ON legal_stories FOR SELECT USING (is_approved = true);
CREATE POLICY "Users can create stories" ON legal_stories FOR INSERT WITH CHECK (auth.uid() = user_id OR user_id IS NULL);
...
CREATE POLICY "Users can create comments" ON story_comments FOR INSERT WITH CHECK (auth.uid() = user_id OR user_id IS NULL);
...
DROP POLICY IF EXISTS "Users can create story likes" ON story_likes;
DROP POLICY IF EXISTS "Users can delete own story likes" ON story_likes;
...
-- Family Law Story
(NULL, 'My Divorce Journey: Lessons Learned', 
...
-- Criminal Law Story
(NULL, 'Wrongful Arrest: How I Fought Back', 
...
-- Property Law Story
(NULL, 'Land Dispute Resolution: A 5-Year Battle', 
...
(NULL, 'Workplace Discrimination: Standing Up for Rights', 
...
(NULL, 'Fighting Against Fraudulent Insurance Claim Denial', 
...
-- Corporate Law Story
(NULL, 'Startup Partnership Dispute: Lessons in Contracts', 
...
-- Tax Law Story
(NULL, 'Tax Assessment Challenge: Understanding Your Rights', 
...
(NULL, 'Visa Extension Battle: Persistence Pays Off', 
...
(NULL, 'Trademark Infringement: Protecting My Brand', 
...
(NULL, 'Fighting Industrial Pollution in My Neighborhood', 
